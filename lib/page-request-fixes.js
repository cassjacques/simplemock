(function(){
    ...
    function fixStyles(sheet) {
      if(sheet.classList.contains('ignore-url-fix'))
        return;
      if (extractHostname(sheet.href).includes('localhost') || extractHostname(sheet.href).includes('simplemock'))
    ...
    function fixScripts(scr) {
      ...
      if (extractHostname(scr.src).includes('localhost') || extractHostname(scr.src).includes('simplemock')) {
    ...
    function fixImages(img) {
      if(img.classList.contains('ignore-url-fix'))
        return;
  
      if (extractHostname(img.src).includes('localhost') || extractHostname(img.src).includes('simplemock'))
        img.src = replaceHost(img.src);
        
    }
  
    console.log("---------");
    console.log("SimpleMock - Init: Page Request Fixes - Live");
    console.log("---------");
  
    document.querySelectorAll("link").forEach(ss => fixStyles(ss));
    document.querySelectorAll("script").forEach(sc => fixScripts(sc));
    document.querySelectorAll("img").forEach(img => fixImages(img));
  })();